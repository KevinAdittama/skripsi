# 01_trainGPU_transface_adaface.py
# -------------------------------------------------------------
# Tahap 1 (GPU A100): Training TransFace (ViT) + AdaFace
...
# ==========================================
# 4. TRAINING LOOP (GPU A100 Optimized)
# ==========================================


def main():
    p = argparse.ArgumentParser(
        description="Train TransFace + AdaFace on GPU A100")
    ...
    print(f"🚀 TransFace (ViT) + AdaFace Training Start on: {device}")
    print(
    ...
    # Note: AdaFaceHead kita definisikan ulang di sini biar aman tanpa import file luar jika di server susah
    # Tapi karena kita sudah import di atas, kita pakai class AdaFaceHead yang ada di file ini (saya copykan class-nya di atas agar standalone aman)
    ...
    # Variabel Save Best
    best_acc = 0.0
    best_save_path = os.path.join(args.out_dir, "best_transface_adaface.pt")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for img, label in pbar:
            ...
            # --- Mixed Precision Forward (AdaFace) ---
            with autocast():
                features = model(img)
        ...
        if epoch % 5 == 0 or epoch == args.epochs:
            save_name = f"transface_adaface_epoch{epoch}.pt"
