# 01_trainGPU_lafs_adaface.py
# -------------------------------------------------------------
# Tahap 1 (GPU A100): Training LAFS + AdaFace
...
def main():
    p = argparse.ArgumentParser(description="Train LAFS + AdaFace on GPU A100")
    ...
    # LR Standard untuk LAFS/CNN (0.1) biasanya aman, tapi bisa diturunkan jika NaN
    p.add_argument("--lr", type=float, default=0.1, help="Learning Rate Awal")
    ...
    print(f"🚀 LAFS + AdaFace Training Start on: {device}")
    print(
    ...
    # Variabel Save Best
    best_acc = 0.0
    best_save_path = os.path.join(args.out_dir, "best_lafs_adaface.pt")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for img, label in pbar:
            ...
            # Gradient Clipping (Penting untuk LAFS + Adaptive Loss)
            scaler.unscale_(opt)
        ...
        if epoch % 5 == 0 or epoch == args.epochs:
            save_name = f"lafs_adaface_epoch{epoch}.pt"
